<%@ Page Title="ผู้ดูแล | Document Control" Language="C#" MasterPageFile="~/Admin/Navbar.Master" AutoEventWireup="true" CodeBehind="RequestSpecDocType.aspx.cs" Inherits="DocumentControl.Admin.RequestSpecDocType" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="container pt-3 pb-2">
        ...
        <%-- แสดงข้อมูล --%>
        <div class="my-2">
            <asp:GridView ID="GVRequestSpecDocType" runat="server" CssClass="table table-sm table-hover" AutoGenerateColumns="false"
                DataSourceID="SqlDataSourceRequestSpecDocType"
                AllowPaging="true" PageSize="20"
                AllowSorting="true"
                EmptyDataText="ไม่พบข้อมูล" ShowHeaderWhenEmpty="true"
                OnRowDataBound="GVRequestSpecDocType_RowDataBound" OnRowCommand="GVRequestSpecDocType_RowCommand">
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="สถานะ" HeaderStyle-Width="120" SortExpression="Status">
                        <ItemTemplate>
                            <asp:LinkButton ID="BtnStatus" runat="server" CommandName="BtnStatus" CommandArgument='<%# Eval("RequestSpecDocTypeID") %>' OnClientClick="return alertConfirm(this, 'เปลี่ยนสถานะ!', 'ต้องการเปลี่ยนสถานะหรือไม่?', 'question', 'ตกลง!', 'ยกเลิก');"></asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="คำสั่ง" HeaderStyle-Width="100">
                        <ItemTemplate>
                            <asp:ImageButton ID="BtnEdit" CssClass="shadow-hover" runat="server" CommandName="BtnEdit" CommandArgument='<%# Eval("RequestSpecDocTypeID") %>' ImageUrl="~/Image/edit.png" Height="30" ToolTip="แก้ไข" />
                            <asp:ImageButton ID="BtnDelete" CssClass="shadow-hover" runat="server" CommandName="BtnDelete" CommandArgument='<%# Eval("RequestSpecDocTypeID") %>' ImageUrl="~/Image/delete.png" Height="30" ToolTip="ลบ" OnClientClick="return alertConfirm(this, 'ลบข้อมูล!', 'ต้องการลบหรือไม่?', 'question', 'ลบ!', 'ยกเลิก');" />
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                </Columns>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-black">
                        ...
                        <div class="text-center">
                            <asp:HiddenField ID="HFRequestSpecDocTypeID" runat="server" />
    ...
    <%-- SqlDataSource --%>
    <%-- ชนิดของเอกสาร --%>
    <asp:SqlDataSource ID="SqlDataSourceRequestSpecDocType" runat="server" ConnectionString="<%$ ConnectionStrings:TCTV1ConnectionString %>"
        SelectCommand="SELECT RequestSpecDocTypeID, DocTypeName, Status FROM DC_RequestSpecDocType ORDER BY Status DESC, DocTypeName ASC"></asp:SqlDataSource>
</asp:Content>
