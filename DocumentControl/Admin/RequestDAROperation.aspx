<%@ Page Title="ผู้ดูแล | Document Control" Language="C#" MasterPageFile="~/Admin/Navbar.Master" AutoEventWireup="true" CodeBehind="RequestDAROperation.aspx.cs" Inherits="DocumentControl.Admin.RequestDAROperation" Culture="en-US" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="container pt-3 pb-2">
        ...
        <%-- แสดงข้อมูล --%>
        <div class="my-2">
            <asp:GridView ID="GVRequestDAROperation" runat="server" CssClass="table table-sm table-hover" AutoGenerateColumns="false" 
                DataSourceID="SqlDataSourceRequestDAROperation"
                AllowPaging="true" PageSize="20"
                AllowSorting="true" 
                ShowHeaderWhenEmpty="true" EmptyDataText="ไม่พบข้อมูล"
                OnRowDataBound="GVRequestDAROperation_RowDataBound" OnRowCommand="GVRequestDAROperation_RowCommand">
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="สถานะ" HeaderStyle-Width="120" SortExpression="Status">
                        <ItemTemplate>
                            <asp:LinkButton ID="BtnStatus" runat="server" CommandName="BtnStatus" CommandArgument='<%# Eval("RequestDAROperationID") %>' OnClientClick="return alertConfirm(this, 'เปลี่ยนสถานะ!', 'ต้องการเปลี่ยนสถานะหรือไม่?', 'question', 'ตกลง!', 'ยกเลิก');"></asp:LinkButton>
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                    <asp:TemplateField HeaderText="คำสั่ง" HeaderStyle-Width="100">
                        <ItemTemplate>
                            <asp:ImageButton ID="BtnEdit" runat="server" CssClass="shadow-hover" CommandName="BtnEdit" CommandArgument='<%# Eval("RequestDAROperationID") %>' ImageUrl="~/Image/edit.png" Height="30" ToolTip="แก้ไข" />
                            <asp:ImageButton ID="BtnDelete" runat="server" CssClass="shadow-hover" CommandName="BtnDelete" CommandArgument='<%# Eval("RequestDAROperationID") %>' ImageUrl="~/Image/delete.png" Height="30" ToolTip="ลบ" OnClientClick="return alertConfirm(this, 'ลบข้อมูล!', 'ต้องการลบหรือไม่?', 'question', 'ลบ!', 'ยกเลิก');" />
                        </ItemTemplate>
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:TemplateField>
                </Columns>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-black">
                        ...
                        <div class="text-center">
                            <asp:HiddenField ID="HFRequestDAROperationID" runat="server" />
    ...
    <%-- SqlDataSource --%>
    <%-- การดำเนินการ --%>
    <asp:SqlDataSource ID="SqlDataSourceRequestDAROperation" runat="server" ConnectionString="<%$ ConnectionStrings:TCTV1ConnectionString %>"
        SelectCommand="SELECT RequestDAROperationID, OperationName, Status FROM DC_RequestDAROperation ORDER BY Status DESC, OperationName ASC"></asp:SqlDataSource>
</asp:Content>
